 ...
 */
class SidebarSpineItem extends InteractiveElement {
  /**
  ...
  get isChildActive() {
    if (document.querySelector('.sidebar-item.active') !== null) {
      return document.querySelector('.sidebar-item.active').datasetpath === this.path;
    }
    return false;
  }
  ...
  get titleTextElement() {
    return new InteractiveElement(this.element.querySelector('.context-link__title'));
  }
  ...
  registerEventListeners() {
    ...
    document.addEventListener('kestros-sidebar-component-area-update', () => {
      if (window.location.pathname.split('.html')[0] === this.path.split('.html')[0]) {
        this.setActive();
      }
    });

    document.addEventListener('kestros-sidebar-component-area-update', event => {
      if (this.path !== null && typeof this.path !== 'undefined') {
        if (this.path.split('.html')[0] !== '/libs/kestros/site-admin') {
          if (window.location.pathname.split('.html')[0].startsWith(this.path.split('.html')[0])) {
            const newSidebarContent = event.detail !== null && typeof event.detail !== 'undefined' && event.detail.path !== '/libs/kestros/site-admin/jcr:content/sidebar/content-area.html';
    ...
    this.element.addEventListener('click', event => {
      ...
      if (!this.disabled && !this.isChildActive) {
        ...
        if (this.submenu) {
          ...
          document.dispatchEvent(new CustomEvent('kestros-sidebar-component-area-update', {
            detail: {
  ...
  setActive() {
    ...
    if (this.validationBadgeGroupElement !== null) {
      this.validationBadgeGroupElement.dispatchEvent(new CustomEvent(ValidationBadgeGroup.events.FILL_ALL));
    }
  }
  ...
  setInactive() {
    ...
    if (this.validationBadgeGroupElement !== null) {
      this.validationBadgeGroupElement.dispatchEvent(new CustomEvent(ValidationBadgeGroup.events.MUTE_ALL));
    }
  }
}