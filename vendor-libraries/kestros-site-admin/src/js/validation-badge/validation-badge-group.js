...
import { Badge } from 'kestros-ui-foundation/src/js/badge/badge'
  ...
  fetchData () {
    fetch(this.path, {
      ...
      .then((res) => {
        ...
        this.dispatchEventOnBadge(this.errorBadgeElement,
          Badge.events.UPDATE_CONTENT, {
            detail: {
              content: errorCount
            }
          })
        this.dispatchEventOnBadge(this.warningBadgeElement,
          Badge.events.UPDATE_CONTENT, {
            detail: {
        ...
        if (errorCount > 0) {
          this.dispatchEventOnBadge(this.errorBadgeElement,
            InteractiveElement.events.SHOW, {})
        }
        if (warningCount > 0) {
          this.dispatchEventOnBadge(this.warningBadgeElement,
            InteractiveElement.events.SHOW, {})
        ...
        console.warn(
          'Failed to load validation badge data for ' + this.path + '.')
      })
  }
