...
import { ValidatedWizard } from './validated-wizard'
import { WizardTabContent } from 'kestros-ui-foundation/src/js/wizard/wizard-tab-content'
import { InputField } from 'kestros-forms/src/js/input-field'

/**
 * Content area corresponding to a ValidatedWizardTab.
 */
export class ValidatedWizardTabContent extends WizardTabContent {
  /**
  ...
  get isValid () {
    for (const inputField of this.inputFieldElements) {
      if (inputField.dataset.valid === false || inputField.dataset.valid ===
          'false') {
        return false
      }
    }
  ...
  registerEventListeners () {
    ...
    for (const inputField of this.inputFieldElements) {
      inputField.addEventListener(InputField.dispatchedEvents.VALIDATION_AFTER,
        (event) => {
    ...
    this.containerElement.addEventListener(
      ValidatedWizard.dispatchedEvents.VALIDATE_TAB, (event) => {
        if (this.containerName === event.detail.wizard) {
          if (this.tabIndex === event.detail.tabIndex) {
            for (const inputField of this.inputFieldElements) {
              inputField.dispatchEvent(
                new CustomEvent(InputField.events.VALIDATE))
            }
      ...
      if (this.isValid) {
        ...
        if (proceedOnComplete) {
          this.containerElement.dispatchEvent(
            new CustomEvent(ValidatedWizard.events.NEXT_TAB))
        }
      } else {
        this.dispatchErrorEvent()
      }
  ...
  dispatchSuccessEvent () {
    this.containerElement.dispatchEvent(
      new CustomEvent(ValidatedWizard.events.TAB_CONTENT_SUCCESS,
        this.eventDetails))
  }
  ...
  dispatchErrorEvent () {
    this.containerElement.dispatchEvent(
      new CustomEvent(ValidatedWizard.events.TAB_CONTENT_ERROR,
        this.eventDetails))
  }
  ...
  dispatchWarningEvent () {
    this.containerElement.dispatchEvent(
      new CustomEvent(ValidatedWizard.events.TAB_CONTENT_WARNING,
        this.eventDetails))
  }
}
