...
class DynamicApplication {
  ...
  initializeInteractiveElements(element) {
    if (element !== null && typeof element !== 'undefined') {
      for (const interactiveElementType of this.interactiveElementTypes) {
        for (const contentElement of element.querySelectorAll(interactiveElementType.selector)) {
          if (!contentElement.dataset.registered) {
            ...
            if (this.isDebugMode) {
              contentElement.setAttribute('data-registered-as', interactiveElementType.type.name);
            }
          }
        }
      }
  ...
  registerInteractiveElementType(selector, type) {
    this.interactiveElementTypes.push(new InteractiveElementType(selector, type));
    this.interactiveElementTypes.sort(this.specificityComperator());
  }
