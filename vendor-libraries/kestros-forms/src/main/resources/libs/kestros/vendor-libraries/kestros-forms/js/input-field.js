 ...
 */
class InputField extends InteractiveElement {
  /**
  ...
  constructor(element) {
    ...
    const validationAlertElement = this.element.querySelector('.input-validation-message');
   ...
   */
  get isValid() {
    return this.element.dataset.valid === 'true' || this.element.dataset.valid === true;
  }
  ...
  registerEventListeners() {
    ...
    this.element.addEventListener(InputField.events.FIELD_INVALID, event => {
      ...
      this.formElement.dispatchEvent(new CustomEvent(Form.events.FIELD_INVALID, event));
    });
  ...
  validate() {
    ...
    if (this.isValid) {
      this.element.dispatchEvent(new CustomEvent(InputField.dispatchedEvents.RUN_VALIDATORS, {
        detail: {
          value: this.value
        }
      }));
