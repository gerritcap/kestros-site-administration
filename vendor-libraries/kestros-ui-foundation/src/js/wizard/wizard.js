...
export class Wizard extends TabsContainer {
   ...
   */
  get disableNext () {
    return Boolean(this.element.dataset.disableNext === 'true' ||
        this.element.dataset.disableNext === true)
  }
    ...
    if (!this.disableNext) {
      if (this.currentTabIndex !== this.tabCount - 1) {
        ...
        this.element.dispatchEvent(
          new CustomEvent(Tab.events.TAB_ENABLE, this.event))
        this.element.dispatchEvent(
          new CustomEvent(TabsContainer.events.TAB_SELECTED, this.event))
        if (this.currentTabIndex === this.tabCount - 1) {
          this.element.dispatchEvent(
            new CustomEvent(Wizard.dispatchedEvents.SELECTED_FINAL_TAB))
        }
      }
    }
  }
  ...
  selectPreviousTab () {
    if (this.currentTabIndex !== 0) {
      ...
      this.element.dispatchEvent(
        new CustomEvent(TabsContainer.events.TAB_SELECTED, this.event))
      this.element.dispatchEvent(
        new CustomEvent(Wizard.dispatchedEvents.DESELECTED_FINAL_TAB))
    }
  }
}
